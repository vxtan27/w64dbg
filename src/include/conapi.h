...
NTSTATUS WriteConsoleDevice(
    ...
    CONSOLE_MSG_WRITECONSOLE_L1 Msg = {{ConsolepWriteConsole,
        sizeof(CONSOLE_WRITECONSOLE_MSG)}, {.Unicode=(BOOLEAN) fUnicode}};
...
NTSTATUS SetConsoleDeviceMode(
    ...
    // First Input Message Structure
    CONSOLE_MSG_MODE_L1 Msg = {{ConsolepSetMode,
        sizeof(CONSOLE_MODE_MSG)}, {dwMode}};
...
NTSTATUS SetConsoleDeviceOutputCP(
    ...
    // First Input Message Structure
    CONSOLE_MSG_SETCP_L2 Msg = {{ConsolepSetCP,
        sizeof(CONSOLE_SETCP_MSG)}, {wCodePageID, TRUE}};
    UDIo.Buffers[0].Size = sizeof(Msg);
    UDIo.Buffers[0].Buffer = &Msg;
