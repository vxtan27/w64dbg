...
static __FORCEINLINE int atou(char *p)
{
    if (*p == '-')
    {
        if (*(p + 1) == '1' && *(p + 2) == '\0')
...
static inline char *debug_ultoa(unsigned int value, char *p)
{
    ...
    *(--ptr) = (value % 10) + '0';
    ...
    if (value >= 10)
    {
        value /= 10;
        *(--ptr) = (value % 10) + '0';
        if (value >= 10) *(--ptr) = (char) (value / 10) + '0';
    }
...
static __FORCEINLINE void space_ultoa(unsigned int value, char *p)
{
    ...
    *(--p) = (value % 10) + '0';
    ...
    if (value >= 10)
    {
        value /= 10;
        *(--p) = (value % 10) + '0';
        if (value >= 10) *(--p) = (char) (value / 10) + '0';
    }
}

static inline char *ulltoaddr(unsigned long long value, char *p, BOOL bWow64)
{
    *p++ = '0';
    *p++ = 'x';
