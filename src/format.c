...
static inline char *FormatDebugException(const EXCEPTION_RECORD *ExceptionRecord, char *p, BOOL bWow64)
{
    switch (ExceptionRecord->ExceptionCode)
    {
        ...
        case 0xC0000006: // EXCEPTION_IN_PAGE_ERROR
            ...
            *p++ = ' ';
            ...
            p = ulltoaddr((ULONGLONG) ExceptionRecord->ExceptionInformation[1], p + 10, bWow64);
            break;
        ...
        case 0x40010003: // DBG_TERMINATE_THREAD
            memcpy(p, "terminate thread", 16);
            p += 16;
            break;
        case 0x40010004: // DBG_TERMINATE_PROCESS
            memcpy(p, "terminate process", 17);
            p += 17;
            break;
        case 0x40010005: // DBG_CONTROL_C
            memcpy(p, "CONTROL+C", 9);
            p += 9;
            break;
        case 0x40010008: // DBG_CONTROL_BREAK
            memcpy(p, "CONTROL+BREAK", 13);
            p += 13;
            break;
...
static __FORCEINLINE char *FormatFileLine(char *fname, unsigned int lnum, size_t len, char *p, char Color)
{
    memcpy(p, fname, len);
    p += len;
...
static inline char *FormatSourceCode(char *fname, unsigned int lnum, char *p, char verbose)
{
    HANDLE hFile = CreateFileA(fname, GENERIC_READ, 0,
    ...
    if (hFile != INVALID_HANDLE_VALUE)
    {
        ...
        while (TRUE)
        {
            if ((ReadFile(hFile, buffer, BUFLEN, &dwRead, NULL) == 0) || !dwRead) break;
            ptr = buffer;

            while ((ptr = (char *) memchr(ptr, '\n', buffer + dwRead - ptr) + 1) > (char *) 1)
            {
                if (++line == lnum)
                {
                    char *_ptr;
                    ...
                    {
                        temp = buffer + dwRead - ptr;
                        memcpy(p, ptr, temp);
                        p += temp;
                        if ((ReadFile(hFile, buffer, BUFLEN, &dwRead, NULL) == 0) || !dwRead) break;
                        ptr = buffer;
                        _ptr = (char *) memchr(buffer, '\n', dwRead);
                        if (_ptr) temp = buffer + dwRead - _ptr;
                        else temp = dwRead;
                    }
