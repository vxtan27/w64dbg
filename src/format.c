...
static inline char *FormatDebugException(const EXCEPTION_RECORD *ExceptionRecord, char *p, char bWow64)
{
...
static __FORCEINLINE char *FormatFileLine(wchar_t *fname, unsigned int lnum, ULONG len, char *p, char Color)
#else
static __FORCEINLINE char *FormatFileLine(wchar_t *fname, unsigned int lnum, int len, char *p, char Color)
#endif
{
...
static inline char *FormatSourceCode(wchar_t *fname,
    unsigned int lnum, char *p, char verbose)
{
    HANDLE hFile;
